...
sat_ownership %>%
  ...
  summarise(size = n()) %>%
  ungroup() %>%
  # mutate(sets = str_split(users, "/")) %>%
  # select(-users) ->
  rename(sets = users) ->
sat_group_data

write_csv(sat_group_data, here("data", "sat-user-overlap-data.csv"))
write_csv(
  tibble(country = unique(sat_group_data$country_owner)),
  here("data", "sat-user-overlap-countries.csv"))

# ojs_define(satGroupData = sat_group_data)
# ojs_define(countryList = unique(sat_group_data$country_owner))
```
